...
const TaskManager: React.FC<TaskManagerProps> = ({ 
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-2 px-1">
        ...
        <div className="flex items-center gap-2">
            <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
                 <button onClick={() => setViewMode('list')} className={`px-3 py-1 rounded-lg text-xs font-bold ${viewMode === 'list' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400'}`}>רשימה</button>
                 <button onClick={() => setViewMode('schedule')} className={`px-3 py-1 rounded-lg text-xs font-bold ${viewMode === 'schedule' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400'}`}>לו"ז</button>
            </div>
      ...
      {isAdding && (
        <div className="bg-white p-6 rounded-3xl border-2 border-indigo-200 shadow-xl space-y-5 animate-in fade-in zoom-in duration-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-400 block">תזכורת מותאמת</label>
              <div className="flex gap-2 items-center bg-slate-50 p-1 rounded-xl border border-slate-200">
                <input type="number" placeholder="זמן" value={customReminderValue} onChange={e => setCustomReminderValue(e.target.value)} className="w-16 p-1 text-center font-bold border-b-2 border-slate-200 focus:border-indigo-500 outline-none bg-transparent" />
                <select value={customReminderUnit} onChange={e => setCustomReminderUnit(e.target.value as any)} className="flex-1 bg-transparent font-bold text-sm text-indigo-800 focus:outline-none p-1">
           ...
           <div className="space-y-3">
            <label className="text-xs font-bold text-slate-400 block">כמה "כוח מוח" זה דורש?</label>
            <div className="flex gap-2">
                {(['low', 'medium', 'high'] as EnergyLevel[]).map(level => (
                    <button key={level} onClick={() => setSelectedEnergy(level)} className={`flex-1 py-2 rounded-xl text-xs font-bold border-2 transition-all flex items-center justify-center gap-1 ${ selectedEnergy === level ? 'bg-indigo-600 text-white border-indigo-600' : 'border-slate-100 text-slate-400' }`}>
      ...
      {viewMode === 'list' ? (
        <div className="space-y-3">
          {filteredTasks.filter(t => !t.completed).sort((a,b) => a.dueDate.getTime() - b.dueDate.getTime()).map(task => {
                   ...
                   <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs shrink-0">{CATEGORY_ICONS[task.category]}</span>
                       <div className="flex items-center gap-1 text-[10px] text-slate-400 shrink-0">
                        <span>⏰</span>
                        <span>{task.dueDate.toLocaleString('he-IL', { weekday: 'short', hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
                       {task.energyLevel && <span className="text-xs shrink-0" title={`רמת אנרגיה: ${task.energyLevel}`}>{ENERGY_LEVEL_ICONS[task.energyLevel]}</span>}
          ...
          {filteredTasks.filter(t => t.completed).length > 0 && (
              <div className="pt-4 mt-4 border-t-2 border-dashed border-slate-200">
                  ...
                  {filteredTasks.filter(t => t.completed).sort((a,b) => b.dueDate.getTime() - a.dueDate.getTime()).map(task => (
                       <button key={task.id} onClick={() => onViewTask(task)} className={`w-full text-start bg-white p-3 rounded-2xl border border-slate-100 flex items-center justify-start gap-4 transition-all hover:shadow-sm group opacity-50 my-2`}>
                          <div onClick={(e) => { e.stopPropagation(); onComplete(task.id); }} className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors shrink-0 cursor-pointer bg-emerald-500 border-emerald-500 text-white`}>✓</div>
        ...
        <DailySchedule tasks={todayTasks} onComplete={onComplete} />
      )}
    </div>
  );
};
